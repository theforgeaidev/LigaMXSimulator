// #region atlante
const atlante = {
    "Director Técnico": [{ name: "Miguel Herrera", dorsal: 90 }],
    Arqueros: [
        { name: "David Ospina", dorsal: 91 },
        { name: "Oscar Jimenez", dorsal: 1 },
        { name: "Roberto Barragán", dorsal: 22 }
    ],
    Defensores: [
        { name: "Luis Sanchez", pos: "DC", dorsal: 3 },
        { name: "Eduardo Tercero", pos: "DC", dorsal: 4 },
        { name: "Nico Carrera", pos: "DC", dorsal: 6 },
        { name: "Christopher Alfaro", pos: "DC", dorsal: 14 },
        { name: "Axl Padilla", pos: "LI", dorsal: 23 },
        { name: "Armando Escobar", pos: "LI", dorsal: 27 },
        { name: "Walter Portales", pos: "LI", dorsal: 29 },
        { name: "Diogo Bagui", pos: "DC", dorsal: 32 },
        { name: "Walter Clar", pos: "LI", dorsal: 37 }
    ],
    Mediocampistas: [
        { name: "Martin Fernandez", pos: "MCD", dorsal: 92 },
        { name: "Alejandro Urzua", pos: "MC", dorsal: 93 },
        { name: "Hardy Meza", pos: "MCD", dorsal: 5 },
        { name: "Jhojan Julio", pos: "MCO", dorsal: 7 },
        { name: "Octavio Vázquez", pos: "MCO", dorsal: 10 },
        { name: "Maximiliano García", pos: "MCO", dorsal: 13 },
        { name: "Leonardo Mejía", pos: "MC", dorsal: 17 },
        { name: "Christian Bermúdez", pos: "MCO", dorsal: 18 },
        { name: "Javier Ibarra", pos: "MCO", dorsal: 21 },
        { name: "Eugenio Pizzuto", pos: "MCD", dorsal: 24 },
        { name: "Diego Pérez", pos: "MC", dorsal: 29 }
    ],
    Delanteros: [
        { name: "Luis Puente", pos: "DC", dorsal: 9 },
        { name: "Joaquín Moxica", pos: "DC", dorsal: 11 },
        { name: "Jairón Charcopa", pos: "DC", dorsal: 15 },
        { name: "José González", pos: "ED", dorsal: 16 },
        { name: "Gilberto Adame", pos: "ED", dorsal: 20 },
        { name: "Luis Calzadilla", pos: "EI", dorsal: 22 },
        { name: "Edgar Jimenez", pos: "ED", dorsal: 34 },
        { name: "Rubén Coubert", pos: "DC", dorsal: 184 }
    ]
};
// #endregion

// #region atleticoSanLuis
const atleticoSanLuis = {
    "Director Técnico": [{ name: "Diego Mejía", dorsal: 90 }],
    Arqueros: [
        { name: "Diego Urtiaga", dorsal: 91 },
        { name: "Andrés Sánchez", dorsal: 1 },
        { name: "César López", dorsal: 23 },
        { name: "Gibrán Lajud", dorsal: 25 }
    ],
    Defensores: [
        { name: "Luis Carrillo", pos: "DC", dorsal: 92 },
        { name: "Román Torres", pos: "LD", dorsal: 2 },
        { name: "Robson Bambu", pos: "DC", dorsal: 3 },
        { name: "Julio César Domínguez", pos: "DC", dorsal: 4 },
        { name: "Juanpe", pos: "DC", dorsal: 6 },
        { name: "Lucas Esteves", pos: "LI", dorsal: 16 },
        { name: "Aldo Cruz", pos: "LI", dorsal: 18 },
        { name: "Benjamín Galindo", pos: "DC", dorsal: 30 },
        { name: "Eduardo Águila", pos: "DC", dorsal: 31 }
    ],
    Mediocampistas: [
        { name: "Johan Caicedo", pos: "MCD", dorsal: 93 },
        { name: "Roberto Meraz", pos: "MCD", dorsal: 5 },
        { name: "Sebastien Salles-Lamonge", pos: "MC", dorsal: 10 },
        { name: "David Rodriguez", pos: "MCO", dorsal: 11 },
        { name: "Miguel Garcia", pos: "MC", dorsal: 14 },
        { name: "Óscar Macías", pos: "MCO", dorsal: 21 },
        { name: "Sebastián Pérez-Bouquet", pos: "MCO", dorsal: 26 },
        { name: "Kevin Ortega", pos: "MCD", dorsal: 35 }
    ],
    Delanteros: [
        { name: "Benjamin Galdames", pos: "ED", dorsal: 7 },
        { name: "Ronaldo Nájera", pos: "EI", dorsal: 8 },
        { name: "João Pedro", pos: "DC", dorsal: 9 },
        { name: "Anderson Duarte", pos: "ED", dorsal: 17 },
        { name: "Santiago Muñóz", pos: "DC", dorsal: 19 },
        { name: "Leonardo Flores", pos: "DC", dorsal: 20 },
        { name: "Rafael Llorente", pos: "ED", dorsal: 22 },
        { name: "Jesús Medina", pos: "ED", dorsal: 28 }
    ]
};
// #endregion

// #region chivas
const chivas = {
    "Director Técnico": [{ name: "Gabriel Milito", dorsal: 90 }],
    Arqueros: [
        { name: "Raúl Rangel", dorsal: 1 },
        { name: "Óscar Whalley", dorsal: 13 },
        { name: "Sebastián Liceaga", dorsal: 31 }
    ],
    Defensores: [
        { name: "José Castillo", pos: "LD", dorsal: 2 },
        { name: "Diego Campillo", pos: "DC", dorsal: 3 },
        { name: "Miguel Tapias", pos: "DC", dorsal: 4 },
        { name: "Bryan González", pos: "LI", dorsal: 5 },
        { name: "Luis Rey", pos: "DC", dorsal: 16 },
        { name: "Daniel Aguirre", pos: "DC", dorsal: 23 },
        { name: "Miguel Gómez", pos: "LD", dorsal: 24 },
        { name: "Leonardo Sepúlveda", pos: "LI", dorsal: 27 },
        { name: "Richard Ledezma", pos: "LD", dorsal: 37 }
    ],
    Mediocampistas: [
        { name: "Omar Govea", pos: "MCD", dorsal: 6 },
        { name: "Luis Romo", pos: "MC", dorsal: 7 },
        { name: "Efraín Álvarez", pos: "MCO", dorsal: 10 },
        { name: "Brian Gutiérrez", pos: "MCO", dorsal: 11 },
        { name: "Kevin Castañeda", pos: "MCO", dorsal: 19 },
        { name: "Fernando González", pos: "MCD", dorsal: 28 },
        { name: "Santiago Sandoval", pos: "MCO", dorsal: 30 }
    ],
    Delanteros: [
        { name: "Ángel Sepúlveda", pos: "DC", dorsal: 9 },
        { name: "Hugo Camberos", pos: "ED", dorsal: 14 },
        { name: "Ricardo Marín", pos: "DC", dorsal: 17 },
        { name: "Jonathan Pérez", pos: "EI", dorsal: 18 },
        { name: "Roberto Alvarado", pos: "ED", dorsal: 25 },
        { name: "Jordán Carrillo", pos: "EI", dorsal: 33 },
        { name: "Armando González", pos: "DC", dorsal: 34 }
    ]
};
// #endregion

// #region clubAmerica
const clubAmerica = {
    "Director Técnico": [{ name: "Guillermo Almada", dorsal: 90 }],
    Arqueros: [
        { name: "Luis Malagon", dorsal: 1 },
        { name: "Fernando Tapia", dorsal: 21 },
        { name: "Rodolfo Cota", dorsal: 30 },
        { name: "Cesar Lugo", dorsal: 31 }
    ],
    Defensores: [
        { name: "Emilio Lara", pos: "LD", dorsal: 2 },
        { name: "Israel Reyes", pos: "LD", dorsal: 3 },
        { name: "Sebastian Caceres", pos: "DC", dorsal: 4 },
        { name: "Kevin Alvarez", pos: "LD", dorsal: 5 },
        { name: "Ralph Orquin", pos: "LD", dorsal: 15 },
        { name: "Aaron Mejia", pos: "LD", dorsal: 18 },
        { name: "Franco Rossano", pos: "LI", dorsal: 24 },
        { name: "Cristian Borja", pos: "LI", dorsal: 26 },
        { name: "Ramon Juarez", pos: "DC", dorsal: 29 },
        { name: "Miguel Vazquez", pos: "DC", dorsal: 32 },
        { name: "Dagoberto Espinoza", pos: "LD", dorsal: 34 }
    ],
    Mediocampistas: [
        { name: "Alan Cervantes", pos: "MCD", dorsal: 13 },
        { name: "Rodrigo Dourado", pos: "MCD", dorsal: 17 },
        { name: "Alexis Gutiérrez", pos: "MCO", dorsal: 20 },
        { name: "Raphael Veiga", pos: "MCO", dorsal: 23 },
        { name: "Érick Sánchez", pos: "MC", dorsal: 28 },
        { name: "Santiago Naveda", pos: "MCD", dorsal: 35 },
        { name: "Miguel Ramírez", pos: "MC", dorsal: 36 },
        { name: "Lima", pos: "MCO", dorsal: 45 }
    ],
    Delanteros: [
        { name: "Brian Rodríguez", pos: "EI", dorsal: 7 },
        { name: "Henry Martin", pos: "DC", dorsal: 9 },
        { name: "Álex Zendejas", pos: "ED", dorsal: 10 },
        { name: "Victor Dávila", pos: "DC", dorsal: 11 },
        { name: "Isaías Violante", pos: "EI", dorsal: 12 },
        { name: "José Raúl Zúñiga", pos: "DC", dorsal: 19 },
        { name: "Patricio Salas", pos: "DC", dorsal: 33 },
        { name: "Esteban Lozano", pos: "DC", dorsal: 37 }
    ]
};
// #endregion

// #region cruzAzul
const cruzAzul = {
    "Director Técnico": [{ name: "Joel Huiqui", dorsal: 90 }],
    Arqueros: [
        { name: "Andrés Gudiño", dorsal: 1 },
        { name: "Kevin Mier", dorsal: 23 },
        { name: "Emmanuel Ochoa", dorsal: 30 }
    ],
    Defensores: [
        { name: "Omar Campos", pos: "LI", dorsal: 3 },
        { name: "Willer Ditta", pos: "DC", dorsal: 4 },
        { name: "Jesus Orozco Chiquete", pos: "DC", dorsal: 5 },
        { name: "Alán Montes", pos: "DC", dorsal: 12 },
        { name: "Jorge Rodarte", pos: "LD", dorsal: 22 },
        { name: "Fernando Sámano", pos: "LD", dorsal: 25 },
        { name: "Raymundo Rubio", pos: "DC", dorsal: 28 },
        { name: "Cristian Jiménez", pos: "DC", dorsal: 32 },
        { name: "Gonzalo Piovi", pos: "DC", dorsal: 33 }
    ],
    Mediocampistas: [
        { name: "Érik Lira", pos: "MCD", dorsal: 6 },
        { name: "Agustín Palavecino", pos: "MC", dorsal: 8 },
        { name: "Andres Montaño", pos: "MC", dorsal: 10 },
        { name: "Jeremy Marquez", pos: "MC", dorsal: 16 },
        { name: "Amaury García", pos: "MCD", dorsal: 17 },
        { name: "Carlos Rodríguez", pos: "MC", dorsal: 19 },
        { name: "José Paradela", pos: "MCO", dorsal: 20 }
    ],
    Delanteros: [
        { name: "Nicolás Ibañez", pos: "DC", dorsal: 9 },
        { name: "Christian Ebere", pos: "DC", dorsal: 11 },
        { name: "Juan Calero", pos: "DC", dorsal: 14 },
        { name: "Luka Romero", pos: "ED", dorsal: 18 },
        { name: "Gabriel Fernandez", pos: "DC", dorsal: 21 },
        { name: "Bryan Gamboa", pos: "DC", dorsal: 27 },
        { name: "Carlos Rotondi", pos: "EI", dorsal: 29 },
        { name: "Amaury Morales", pos: "ED", dorsal: 194 },
        { name: "Mateo Levy", pos: "DC", dorsal: 214 }
    ]
};
// #endregion

// #region leon
const leon = {
    "Director Técnico": [{ name: "Javier Gandolfi", dorsal: 90 }],
    Arqueros: [
        { name: "Jordan Garcia", dorsal: 1 },
        { name: "Oscar García", dorsal: 23 }
    ],
    Defensores: [
        { name: "Jhohan Romaña", pos: "DC", dorsal: 91 },
        { name: "Juan Guevara", pos: "DC", dorsal: 92 },
        { name: "Valentín Gauthier", pos: "DC", dorsal: 2 },
        { name: "Bryan Colula", pos: "LD", dorsal: 4 },
        { name: "Sebastian Vegas", pos: "DC", dorsal: 5 },
        { name: "Iván Moreno", pos: "LD", dorsal: 7 },
        { name: "Paul Bellón", pos: "DC", dorsal: 25 },
        { name: "Salvador Reyes", pos: "LI", dorsal: 26 },
        { name: "David Ramírez", pos: "LD", dorsal: 28 },
        { name: "Abraham Villegas", pos: "LI", dorsal: 33 }
    ],
    Mediocampistas: [
        { name: "Santiago Colombatto", pos: "MCD", dorsal: 93 },
        { name: "Nicolás Fonseca", pos: "MCD", dorsal: 94 },
        { name: "Sebastian Fierro", pos: "MC", dorsal: 95 },
        { name: "Sebastian Santos", pos: "MC", dorsal: 96 },
        { name: "Luis Valadez", pos: "MC", dorsal: 97 },
        { name: "Fernando Beltran", pos: "MC", dorsal: 6 },
        { name: "Daniel Arcila", pos: "MCO", dorsal: 13 },
        { name: "Rodrigo Echeverría", pos: "MCD", dorsal: 20 },
        { name: "Ángel Estrada", pos: "MCO", dorsal: 24 },
        { name: "Iván Rodríguez", pos: "MCD", dorsal: 29 }
    ],
    Delanteros: [
        { name: "Federico Viñas", pos: "DC", dorsal: 98 },
        { name: "Edgar Guerra", pos: "ED", dorsal: 99 },
        { name: "Juan Domínguez", pos: "ED", dorsal: 8 },
        { name: "Ismael Díaz", pos: "EI", dorsal: 11 },
        { name: "Jordi Cortizo", pos: "ED", dorsal: 16 },
        { name: "Gael García", pos: "EI", dorsal: 17 },
        { name: "Rogelio Funes Mori", pos: "DC", dorsal: 18 },
        { name: "Alfonso Alvarado", pos: "DC", dorsal: 19 },
        { name: "Diber Cambindo", pos: "DC", dorsal: 27 }
    ]
};
// #endregion

// #region monterrey
const monterrey = {
    "Director Técnico": [{ name: "Matias Almeyda", dorsal: 90 }],
    Arqueros: [
        { name: "Esteban Andrada", dorsal: 91 },
        { name: "César Ramos", dorsal: 92 },
        { name: "Luis Cardenas", dorsal: 22 },
        { name: "Santiago Mele", dorsal: 25 }
    ],
    Defensores: [
        { name: "Javier Casillas", pos: "LD", dorsal: 93 },
        { name: "Ricardo Chávez", pos: "LD", dorsal: 2 },
        { name: "Gerardo Arteaga", pos: "LI", dorsal: 3 },
        { name: "Victor Guzmán", pos: "DC", dorsal: 4 },
        { name: "Carlos Salcedo", pos: "DC", dorsal: 13 },
        { name: "Erick Aguirre", pos: "LD", dorsal: 14 },
        { name: "Alonso Aceves", pos: "LI", dorsal: 19 },
        { name: "Luis Reyes", pos: "LI", dorsal: 21 },
        { name: "Stefan Medina", pos: "LD", dorsal: 33 },
        { name: "César Bustos", pos: "DC", dorsal: 34 }
    ],
    Mediocampistas: [
        { name: "Orbelín Pineda", pos: "MC", dorsal: 94 },
        { name: "César Garza", pos: "MCD", dorsal: 95 },
        { name: "Fidel Ambriz", pos: "MCD", dorsal: 5 },
        { name: "Oliver Torres", pos: "MC", dorsal: 8 },
        { name: "Jorge Rodriguez", pos: "MCD", dorsal: 30 },
        { name: "Sebastian Rodriguez", pos: "MC", dorsal: 32 },
        { name: "Iker Fimbres", pos: "MCO", dorsal: 35 }
    ],
    Delanteros: [
        { name: "Hugo Cuypers", pos: "DC", dorsal: 96 },
        { name: "Diego Rossi", pos: "DC", dorsal: 97 },
        { name: "Luca Orellano", pos: "ED", dorsal: 11 },
        { name: "Jesus Corona", pos: "ED", dorsal: 17 },
        { name: "Uros Djurdjevic", pos: "DC", dorsal: 20 },
        { name: "Roberto de la Rosa", pos: "DC", dorsal: 27 },
        { name: "Lucas Ocampos", pos: "EI", dorsal: 29 }
    ]
};
// #endregion

// #region pachuca
const pachuca = {
    "Director Técnico": [{ name: "Benjamin Mora", dorsal: 90 }],
    Arqueros: [
        { name: "Carlos Moreno", dorsal: 25 },
        { name: "José Eulogio", dorsal: 31 }
    ],
    Defensores: [
        { name: "Francisco Venegas", pos: "LI", dorsal: 91 },
        { name: "Mauricio Isais", pos: "LI", dorsal: 92 },
        { name: "Sergio Barreto", pos: "DC", dorsal: 2 },
        { name: "Eduardo Bauermann", pos: "DC", dorsal: 4 },
        { name: "Jorge Berlanga", pos: "DC", dorsal: 13 },
        { name: "Carlos Sánchez", pos: "LD", dorsal: 14 },
        { name: "René López", pos: "LD", dorsal: 21 },
        { name: "Alan Mozo", pos: "LD", dorsal: 22 },
        { name: "Andrés Micolta", pos: "DC", dorsal: 33 },
        { name: "Pedro Martínez", pos: "DC", dorsal: 35 }
    ],
    Mediocampistas: [
        { name: "Rodolfo Pizarro", pos: "MCO", dorsal: 93 },
        { name: "Sergio Rodríguez", pos: "MC", dorsal: 94 },
        { name: "Israel Luna", pos: "MCO", dorsal: 15 },
        { name: "Christian Rivera", pos: "MCD", dorsal: 16 },
        { name: "Alan Bautista", pos: "MCO", dorsal: 26 },
        { name: "Elias Montiel", pos: "MCD", dorsal: 28 }
    ],
    Delanteros: [
        { name: "Kenedy", pos: "EI", dorsal: 95 },
        { name: "Illian Hernández", pos: "DC", dorsal: 96 },
        { name: "Adrian Alcaraz", pos: "DC", dorsal: 97 },
        { name: "Nicolas Vallejo", pos: "ED", dorsal: 98 },
        { name: "Oussama Idrissi", pos: "EI", dorsal: 11 },
        { name: "Alexei Domínguez", pos: "ED", dorsal: 18 },
        { name: "Salomon Rondon", pos: "DC", dorsal: 23 }
    ]
};
// #endregion

// #region tigres
const tigres = {
    "Director Técnico": [{ name: "Guido Pizarro", dorsal: 90 }],
    Arqueros: [
        { name: "Nahuel Guzman", dorsal: 1 },
        { name: "Antonio Carrera", dorsal: 13 },
        { name: "Felipe Rodriguez", dorsal: 25 }
    ],
    Defensores: [
        { name: "Alan Franco", pos: "DC", dorsal: 91 },
        { name: "Francisco Reyes", pos: "DC", dorsal: 92 },
        { name: "Marco Farfan", pos: "LI", dorsal: 3 },
        { name: "Juan Jose Purata", pos: "DC", dorsal: 4 },
        { name: "Jesús Garza", pos: "LD", dorsal: 14 },
        { name: "Fernando Ordóñez", pos: "LI", dorsal: 26 },
        { name: "Jesús Angulo", pos: "DC", dorsal: 27 },
        { name: "Joaquim", pos: "DC", dorsal: 28 },
        { name: "Vladimir Loroña", pos: "LD", dorsal: 32 },
        { name: "Rafael Guerrero", pos: "DC", dorsal: 33 },
        { name: "Osvaldo Rodríguez", pos: "LI", dorsal: 35 }
    ],
    Mediocampistas: [
        { name: "Henrique Simeone", pos: "MCD", dorsal: 93 },
        { name: "Cesar Araujo", pos: "MCD", dorsal: 5 },
        { name: "Juan Pablo Vigon", pos: "MC", dorsal: 6 },
        { name: "Fernando Gorriarán", pos: "MC", dorsal: 8 },
        { name: "Juan Brunetta", pos: "MCO", dorsal: 11 },
        { name: "Rômulo", pos: "MCD", dorsal: 23 }
    ],
    Delanteros: [
        { name: "Angel Correa", pos: "ED", dorsal: 7 },
        { name: "Diego Lainez", pos: "ED", dorsal: 16 },
        { name: "Rodrigo Aguirre", pos: "DC", dorsal: 17 },
        { name: "Marcelo Flores", pos: "EI", dorsal: 20 },
        { name: "Diego Sánchez", pos: "EI", dorsal: 30 },
        { name: "Ozziel Herrera", pos: "ED", dorsal: 77 }
    ]
};
// #endregion

// #region tijuana
const tijuana = {
    "Director Técnico": [{ name: "Sebastian Abreu", dorsal: 90 }],
    Arqueros: [
        { name: "Antonio Rodríguez", dorsal: 2 },
        { name: "Salim Hernández", dorsal: 29 }
    ],
    Defensores: [
        { name: "Rafael Fernandez", pos: "LD", dorsal: 3 },
        { name: "Unai Bilbao", pos: "DC", dorsal: 4 },
        { name: "Alejandro Gomez", pos: "DC", dorsal: 6 },
        { name: "Jackson Porozo", pos: "DC", dorsal: 12 },
        { name: "Alan Vega", pos: "LI", dorsal: 16 },
        { name: "Óscar Manzanarez", pos: "DC", dorsal: 25 },
        { name: "Pablo Ortiz", pos: "LI", dorsal: 33 }
    ],
    Mediocampistas: [
        { name: "Kevin Escamilla", pos: "MCD", dorsal: 5 },
        { name: "Iván Tona", pos: "MCD", dorsal: 8 },
        { name: "Gilberto Mora", pos: "MCO", dorsal: 10 },
        { name: "Ignacio Rivero", pos: "MC", dorsal: 15 },
        { name: "Ramiro Árciga", pos: "MC", dorsal: 17 },
        { name: "Ángel Zapata", pos: "MCD", dorsal: 20 },
        { name: "Aldahir Perez", pos: "MCO", dorsal: 26 },
        { name: "Frank Boya", pos: "MCD", dorsal: 34 }
    ],
    Delanteros: [
        { name: "Adonis Preciado", pos: "EI", dorsal: 7 },
        { name: "Mourad El Ghezouani", pos: "DC", dorsal: 9 },
        { name: "Diego Abreu", pos: "DC", dorsal: 19 },
        { name: "Josef Martinez", pos: "DC", dorsal: 30 },
        { name: "Yael Padilla", pos: "EI", dorsal: 31 }
    ]
};
// #endregion

// #region queretaro
const queretaro = {
    "Director Técnico": [{ name: "Esteban González", dorsal: 90 }],
    Arqueros: [
        { name: "Jose Hernández", dorsal: 91 },
        { name: "Guillermo Allison", dorsal: 25 }
    ],
    Defensores: [
        { name: "Paulo Victor", pos: "LI", dorsal: 92 },
        { name: "Omar Mendoza", pos: "LD", dorsal: 2 },
        { name: "Lucas Abascia", pos: "DC", dorsal: 2 },
        { name: "Diego Reyes", pos: "DC", dorsal: 9 },
        { name: "Carlos Villanueva", pos: "DC", dorsal: 15 },
        { name: "Bayron Duarte", pos: "LD", dorsal: 22 },
        { name: "Daniel Parra", pos: "LI", dorsal: 27 }
    ],
    Mediocampistas: [
        { name: "Enzo Giménez", pos: "MC", dorsal: 93 },
        { name: "Juan Cázares", pos: "MC", dorsal: 94 },
        { name: "Christopher Figueroa", pos: "MCO", dorsal: 95 },
        { name: "Carlo García", pos: "MC", dorsal: 4 },
        { name: "Santiago Homenchenko", pos: "MCD", dorsal: 6 },
        { name: "Bernardo Parra", pos: "MCD", dorsal: 8 },
        { name: "Erik Duenas", pos: "MC", dorsal: 18 },
        { name: "Alex Alcala", pos: "MCO", dorsal: 20 },
        { name: "Jesús Arellano", pos: "MC", dorsal: 30 },
        { name: "Mono Martínez", pos: "MCO", dorsal: 30 }
    ],
    Delanteros: [
        { name: "Jean Unjanque", pos: "ED", dorsal: 14 },
        { name: "Juan Robles", pos: "EI", dorsal: 23 },
        { name: "Pérez Reyes", pos: "DC", dorsal: 26 },
        { name: "Waldo Madrid", pos: "ED", dorsal: 29 },
        { name: "Ali Avila", pos: "DC", dorsal: 31 },
        { name: "Mateo Coronel", pos: "DC", dorsal: 37 },
        { name: "Víctor López", pos: "EI", dorsal: 47 }
    ]
};
// #endregion

// #region pumas
const pumas = {
    "Director Técnico": [{ name: "Esteban Solari", dorsal: 90 }],
    Arqueros: [
        { name: "Miguel Paul", dorsal: 91 },
        { name: "Keylor Navas", dorsal: 1 },
        { name: "Pablo Lara", dorsal: 35 },
        { name: "Rodrigo Parra", dorsal: 254 }
    ],
    Defensores: [
        { name: "Cristian Calderón", pos: "LI", dorsal: 92 },
        { name: "Pablo Bennevendo", pos: "LD", dorsal: 2 },
        { name: "Rubén Duarte", pos: "DC", dorsal: 3 },
        { name: "Nathan Silva", pos: "DC", dorsal: 6 },
        { name: "Pablo Monroy", pos: "LD", dorsal: 13 },
        { name: "Jesús Rivas", pos: "LD", dorsal: 19 },
        { name: "Tony Leone", pos: "DC", dorsal: 24 },
        { name: "Álvaro Angulo", pos: "LI", dorsal: 77 },
        { name: "Angel Azuaje", pos: "DC", dorsal: 215 }
    ],
    Mediocampistas: [
        { name: "Sebastián Córdova", pos: "MCO", dorsal: 93 },
        { name: "Piero Quispe", pos: "MCO", dorsal: 94 },
        { name: "Víctor Arteaga", pos: "MCD", dorsal: 95 },
        { name: "Rodrigo López", pos: "MC", dorsal: 7 },
        { name: "Santiago Trigos", pos: "MCD", dorsal: 20 },
        { name: "Ángel Rico", pos: "MCO", dorsal: 26 },
        { name: "Adalberto Carrasquilla", pos: "MC", dorsal: 28 },
        { name: "Pedro Vite", pos: "MC", dorsal: 45 }
    ],
    Delanteros: [
        { name: "Leo Suárez", pos: "ED", dorsal: 96 },
        { name: "Santiago López", pos: "DC", dorsal: 97 },
        { name: "Guillermo Martínez", pos: "DC", dorsal: 9 },
        { name: "José Juan Macías", pos: "DC", dorsal: 11 },
        { name: "Uriel Antuna", pos: "ED", dorsal: 21 },
        { name: "Alan Medina", pos: "ED", dorsal: 22 },
        { name: "Juninho", pos: "DC", dorsal: 23 },
        { name: "Robert Morales", pos: "DC", dorsal: 31 }
    ]
};
// #endregion

// #region necaxa
const necaxa = {
    "Director Técnico": [{ name: "Martín Varini Di Zioli", dorsal: 90 }],
    Arqueros: [
        { name: "Christopher Andrade", dorsal: 91 },
        { name: "Luis Jiménez", dorsal: 12 }
    ],
    Defensores: [
        { name: "Carlos Vargas", pos: "LI", dorsal: 92 },
        { name: "Kaiky Naves", pos: "DC", dorsal: 93 },
        { name: "Mauro Zaleta", pos: "LI", dorsal: 94 },
        { name: "Diego Ochoa", pos: "DC", dorsal: 95 },
        { name: "Emilio Martínez", pos: "LD", dorsal: 2 },
        { name: "Agustín Oliveros", pos: "DC", dorsal: 3 },
        { name: "Alexis Peña", pos: "DC", dorsal: 4 },
        { name: "Francisco Méndez", pos: "DC", dorsal: 20 },
        { name: "Raúl Martínez", pos: "DC", dorsal: 33 }
    ],
    Mediocampistas: [
        { name: "Pedro Pedraza", pos: "MCD", dorsal: 96 },
        { name: "Juan Pablo Torres", pos: "MCO", dorsal: 97 },
        { name: "Matías Espíndola", pos: "MCO", dorsal: 98 },
        { name: "Danny Leyva", pos: "MCD", dorsal: 6 },
        { name: "Lorenzo Faravelli", pos: "MCO", dorsal: 8 },
        { name: "Rogelio Cortéz", pos: "MCO", dorsal: 17 },
        { name: "Israel Tello", pos: "MCO", dorsal: 18 }
    ],
    Delanteros: [
        { name: "Misael Pedroza", pos: "DC", dorsal: 99 },
        { name: "Juan Valencia", pos: "DC", dorsal: 100 },
        { name: "Emilio Rodríguez", pos: "ED", dorsal: 101 },
        { name: "Kevin Rosero", pos: "ED", dorsal: 7 },
        { name: "Julian Carranza", pos: "DC", dorsal: 21 },
        { name: "Ricardo Monreal", pos: "DC", dorsal: 30 },
        { name: "Javier Ruiz", pos: "EI", dorsal: 35 }
    ]
};
// #endregion

// #region toluca
const toluca = {
    "Director Técnico": [{ name: "Antonio Mohamed", dorsal: null }],
    Arqueros: [
        { name: "Hugo González", dorsal: 1 },
        { name: "Ronaldo Beltrán", dorsal: 12 },
        { name: "David Shrem", dorsal: 18 },
        { name: "Luis García", dorsal: 22 }
    ],
    Defensores: [
        { name: "Diego Barbosa", pos: "LD", dorsal: 2 },
        { name: "Antonio Briseño", pos: "DC", dorsal: 3 },
        { name: "Bruno Mendez", pos: "DC", dorsal: 4 },
        { name: "Andres Pereira", pos: "DC", dorsal: 6 },
        { name: "Luan Garcia", pos: "DC", dorsal: 13 },
        { name: "Brian García", pos: "LI", dorsal: 17 },
        { name: "Santiago Simon", pos: "DC", dorsal: 19 },
        { name: "Jesús Gallardo", pos: "LI", dorsal: 20 },
        { name: "Everardo López", pos: "LD", dorsal: 25 }
    ],
    Mediocampistas: [
        { name: "Franco Romero", pos: "MCD", dorsal: 5 },
        { name: "Víctor Guzmán", pos: "MC", dorsal: 7 },
        { name: "Nicolas Castro", pos: "MCO", dorsal: 8 },
        { name: "Jesús Angulo", pos: "MC", dorsal: 10 },
        { name: "Marcel Ruiz", pos: "MC", dorsal: 14 },
        { name: "Pavel Pérez", pos: "MCD", dorsal: 15 },
        { name: "Fernando Arce", pos: "MC", dorsal: 24 },
        { name: "Jorge Díaz", pos: "MCO", dorsal: 29 }
    ],
    Delanteros: [
        { name: "Ernesto Vega", pos: "DC", dorsal: 9 },
        { name: "Helio Nunes", pos: "ED", dorsal: 11 },
        { name: "Edgar López", pos: "ED", dorsal: 21 },
        { name: "Oswaldo Virgen", pos: "EI", dorsal: 23 },
        { name: "Joao Dias", pos: "DC", dorsal: 26 },
        { name: "Erick Gutiérrez", pos: "DC", dorsal: 35 }
    ]
};
// #endregion

// #region santosLaguna
const santosLaguna = {
    "Director Técnico": [{ name: "Renato Alves", dorsal: null }],
    Arqueros: [
        { name: "Carlos Acevedo", dorsal: 1 },
        { name: "Héctor Holguín", dorsal: 33 }
    ],
    Defensores: [
        { name: "Mauricio Cuevas", pos: "LI", dorsal: 3 },
        { name: "José Abella", pos: "DC", dorsal: 4 },
        { name: "Efraín Orona", pos: "DC", dorsal: 14 },
        { name: "Emmanuel Echeverría", pos: "LD", dorsal: 17 },
        { name: "Franco Pardo", pos: "DC", dorsal: 18 },
        { name: "Kevin Picón", pos: "LD", dorsal: 22 },
        { name: "Felipe Sánchez", pos: "DC", dorsal: 25 },
        { name: "Jonathan Pérez", pos: "LI", dorsal: 28 }
    ],
    Mediocampistas: [
        { name: "Aldo López", pos: "MCD", dorsal: 5 },
        { name: "Javier Güemez", pos: "MC", dorsal: 6 },
        { name: "Diego González", pos: "MC", dorsal: 7 },
        { name: "Salvador Mariscal", pos: "MCD", dorsal: 8 },
        { name: "Ezequiel Bullaude", pos: "MCO", dorsal: 10 },
        { name: "Carlos Gruezo", pos: "MCD", dorsal: 11 },
        { name: "Joshua Mancha", pos: "MC", dorsal: 15 },
        { name: "Francisco Villalba", pos: "MCO", dorsal: 21 },
        { name: "Diego Medina", pos: "MC", dorsal: 24 },
        { name: "Ramiro Sordo", pos: "MCO", dorsal: 26 },
        { name: "Kevin Palacios", pos: "MC", dorsal: 77 }
    ],
    Delanteros: [
        { name: "Lucas Di Yorio", pos: "DC", dorsal: 9 },
        { name: "Eduardo Aguirre", pos: "DC", dorsal: 19 },
        { name: "Tahiel Jiménez", pos: "EI", dorsal: 29 }
    ]
};
// #endregion

// #region puebla
const puebla = {
    "Director Técnico": [{ name: "Gerardo Espinoza", dorsal: null }],
    Arqueros: [
        { name: "Ricardo Gutiérrez", dorsal: 28 },
        { name: "Jesús Rodríguez", dorsal: 33 }
    ],
    Defensores: [
        { name: "Ángel Leyva", pos: "LD", dorsal: 2 },
        { name: "Juan Vargas", pos: "DC", dorsal: 4 },
        { name: "Facundo Almada", pos: "DC", dorsal: 5 },
        { name: "Eduardo Navarro", pos: "DC", dorsal: 13 },
        { name: "Oscar Villa", pos: "LI", dorsal: 15 },
        { name: "José Pachuca", pos: "DC", dorsal: 20 }
    ],
    Mediocampistas: [
        { name: "Alberto Herrera", pos: "MC", dorsal: 3 },
        { name: "Fernando Monárrez", pos: "MC", dorsal: 7 },
        { name: "Omar Moreno", pos: "MCD", dorsal: 8 },
        { name: "Mathías Tomas", pos: "MCO", dorsal: 10 },
        { name: "Iker Moreno", pos: "MC", dorsal: 12 },
        { name: "Alonso Ramírez", pos: "MCD", dorsal: 16 },
        { name: "Raúl Castillo", pos: "MC", dorsal: 17 },
        { name: "Sergio Sanabria", pos: "MCO", dorsal: 21 },
        { name: "Carlos Baltazar", pos: "MC", dorsal: 22 },
        { name: "Alejandro Organista", pos: "MCO", dorsal: 24 },
        { name: "Kevin Velasco", pos: "MCD", dorsal: 26 },
        { name: "Brayan Garnica", pos: "MC", dorsal: 27 },
        { name: "Lucas Azevedo", pos: "MC", dorsal: 34 }
    ],
    Delanteros: [
        { name: "Luifer Hernández", pos: "DC", dorsal: 9 },
        { name: "Emiliano Gómez", pos: "ED", dorsal: 11 },
        { name: "Ignacio Maestro", pos: "EI", dorsal: 19 },
        { name: "Eduardo Mustre", pos: "DC", dorsal: 29 }
    ]
};
// #endregion

// #region fcJuarez
const fcJuarez = {
    "Director Técnico": [{ name: "Pedro Faria", dorsal: null }],
    Arqueros: [
        { name: "Sebastián Jurado", dorsal: 1 },
        { name: "Benny Díaz", dorsal: 24 },
        { name: "Guillermo Ruiz", dorsal: 27 }
    ],
    Defensores: [
        { name: "Gilberto Sepúlveda", pos: "LD", dorsal: 2 },
        { name: "Jesús Murillo", pos: "DC", dorsal: 3 },
        { name: "Manuel Mayorga", pos: "DC", dorsal: 4 },
        { name: "Eder López", pos: "LI", dorsal: 14 },
        { name: "Javier Aquino", pos: "LD", dorsal: 20 },
        { name: "Oscar Ortega", pos: "DC", dorsal: 23 },
        { name: "Francisco Nevárez", pos: "DC", dorsal: 33 },
        { name: "Bryan Romero", pos: "LI", dorsal: 35 }
    ],
    Mediocampistas: [
        { name: "Denzell García", pos: "MCD", dorsal: 5 },
        { name: "Ramón Rodríguez", pos: "MC", dorsal: 6 },
        { name: "Raymundo Fulgencio", pos: "MC", dorsal: 7 },
        { name: "Guilherme Castilho", pos: "MCD", dorsal: 8 },
        { name: "Madson De Souza Silva", pos: "MCO", dorsal: 9 },
        { name: "Ían Torres", pos: "MCO", dorsal: 10 },
        { name: "Jose Rodriguez", pos: "MC", dorsal: 11 },
        { name: "Lucas Romero", pos: "MCD", dorsal: 15 },
        { name: "Juan Sigala", pos: "MC", dorsal: 16 },
        { name: "Ricardo Oliveira", pos: "MCO", dorsal: 21 },
        { name: "Said Godínez", pos: "MC", dorsal: 25 }
    ],
    Delanteros: [
        { name: "Luca Martínez", pos: "ED", dorsal: 17 },
        { name: "Oscar Estupiñan", pos: "DC", dorsal: 19 },
        { name: "Ettson Ayón", pos: "EI", dorsal: 29 }
    ]
};
// #endregion

// #region atlas
const atlas = {
    "Director Técnico": [{ name: "Hernán Crespo", dorsal: null }],
    Arqueros: [
        { name: "Camilo Vargas", dorsal: 12 },
        { name: "Antonio Sánchez", dorsal: 22 }
    ],
    Defensores: [
        { name: "Gustavo Ferrareis", pos: "LD", dorsal: 2 },
        { name: "Jorge Sánchez", pos: "LD", dorsal: 3 },
        { name: "Adrián Mora", pos: "DC", dorsal: 4 },
        { name: "Jorge Rodríguez", pos: "DC", dorsal: 5 },
        { name: "Gaddi Aguirre", pos: "DC", dorsal: 13 },
        { name: "Paulo Ramírez", pos: "LI", dorsal: 15 },
        { name: "Milton Valenzuela", pos: "LI", dorsal: 16 },
        { name: "José Lozano", pos: "DC", dorsal: 17 },
        { name: "Rodrigo Schlegel", pos: "DC", dorsal: 21 },
        { name: "Manuel Capasso", pos: "DC", dorsal: 28 }
    ],
    Mediocampistas: [
        { name: "Edgar Zaldivar", pos: "MCD", dorsal: 6 },
        { name: "Luis Leite", pos: "MC", dorsal: 10 },
        { name: "Luis De Barros", pos: "MCO", dorsal: 11 },
        { name: "Sergio Hernández", pos: "MC", dorsal: 23 },
        { name: "Aldo Rocha", pos: "MCO", dorsal: 26 },
        { name: "Víctor Ríos", pos: "MCD", dorsal: 27 },
        { name: "Arturo González", pos: "MC", dorsal: 58 }
    ],
    Delanteros: [
        { name: "Ryan Mmaee", pos: "DC", dorsal: 8 }
    ]
};
// #endregion

const ligaMX = {
    atlante,
    atlas,
    atleticoSanLuis,
    chivas,
    clubAmerica,
    cruzAzul,
    fcJuarez,
    leon,
    monterrey,
    necaxa,
    pachuca,
    puebla,
    pumas,
    queretaro,
    santosLaguna,
    tigres,
    tijuana,
    toluca
};

export { atlante, atlas, atleticoSanLuis, chivas, clubAmerica, cruzAzul, fcJuarez, leon, ligaMX, monterrey, necaxa, pachuca, puebla, pumas, queretaro, santosLaguna, tigres, tijuana, toluca };